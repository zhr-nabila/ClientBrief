...
const dictionary = {
  ID: {
    desc: "Ayo buat ide digital Anda jadi nyata",
    ...
    upload: "Upload Referensi (Multiple Files)",
    back: "Kembali",
    next: "Lanjutkan",
    submit: "Kirim Brief",
  },
  EN: {
    desc: "Let’s make your digital ideas a reality",
    ...
    upload: "Upload Reference (Multiple Files)",
    back: "Back",
    next: "Next",
    submit: "Submit Brief",
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Tambahkan setelah bagian existing code
    ...
    fileInput.addEventListener("change", function () {
      ...
      if (this.files.length > 0) {
        ...
        Array.from(this.files).forEach((file, index) => {
          ...
          div.className =
            "flex items-center justify-between bg-white/5 p-3 rounded-lg";

          div.innerHTML = `
                    <div class="flex items-center gap-3">
                        <i data-lucide="${getFileIcon(file.type)}" class="w-4 h-4 text-blue-500"></i>
        ...
        fileInfo.textContent = `${this.files.length} file(s) selected - Total: ${formatBytes(totalSize)}`;
      } else {
        fileInfo.textContent = "Max 10MB per file";
      }
