...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ===========================
     FAKE SELECT — LAYANAN (FIX)
     =========================== */

  const serviceSelect = document.querySelector(".service-select");
  if (!serviceSelect) return;

  const serviceSelected = serviceSelect.querySelector(".service-selected");
  const serviceText = document.createElement("span");
  serviceText.textContent = serviceSelected.textContent.trim();
  serviceSelected.textContent = "";
  serviceSelected.appendChild(serviceText);
  serviceSelected.insertAdjacentHTML(
    "beforeend",
    '<i data-lucide="chevron-down"></i>'
  );

  const serviceOptions = serviceSelect.querySelectorAll(".service-option");
  const realSelect = document.getElementById("in-jasa");
