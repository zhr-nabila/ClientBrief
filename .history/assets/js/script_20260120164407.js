...
document.addEventListener('DOMContentLoaded', function() {
    // Reset isi option di select jasa (hapus tag <i>)
    const selectJasa = document.getElementById('in-jasa');
    if (selectJasa) {
        // Simpan nilai yang dipilih
        const selectedValue = selectJasa.value;
        
        // Reset options ke teks biasa
        const options = selectJasa.options;
        for (let i = 0; i < options.length; i++) {
            const option = options[i];
            // Hapus tag HTML, ambil hanya teksnya
            const text = option.textContent || option.innerText;
            // Ambil teks tanpa tag
            const cleanText = text.replace(/<[^>]*>/g, '').trim();
            option.textContent = cleanText;
        }
        
        // Kembalikan nilai yang dipilih
        selectJasa.value = selectedValue;
    }
    
    // Juga reset saat toggle language
    const originalToggleLang = toggleLang;
    window.toggleLang = function() {
        originalToggleLang();
        
        // Reset option text setelah ganti bahasa
        setTimeout(() => {
            const selectJasa = document.getElementById('in-jasa');
            if (selectJasa) {
                const options = selectJasa.options;
