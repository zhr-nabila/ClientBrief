...
// Function untuk menangani input changes
function handleInputChanges() {
  ...
  // Special handling for tel input
  const telInput = document.querySelector('#in-telp');
  if (telInput) {
    telInput.addEventListener('input', () => checkTelInputState());
    telInput.addEventListener('change', () => checkTelInputState());
  }
}
...
function checkTelInputState() {
  const telInput = document.querySelector('#in-telp');
  const formGroup = telInput?.closest('.form-group');
  if (!telInput || !formGroup) return;
  
  if (telInput.value.trim() !== '') {
    formGroup.classList.add('filled');
    telInput.classList.add('filled');
  } else {
    formGroup.classList.remove('filled');
    telInput.classList.remove('filled');
  }
}
