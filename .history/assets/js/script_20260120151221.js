  ...
  for (let i = 1; i <= 3; i++) {
    ...
    if (bar)
      bar.style.backgroundColor =
        i <= step ? "#2563eb" : "rgba(255,255,255,0.1)";
  }
}

// Tambahkan efek glow pada progress bar
function updateProgressAnimation(step) {
    for (let i = 1; i <= 3; i++) {
        const bar = document.getElementById("p-" + i);
        if (bar) {
            if (i <= step) {
                // Aktifkan gradien dan animasi
                bar.style.background = "linear-gradient(90deg, #FE7F42, #FFF897)";
                bar.style.boxShadow = "0 0 15px rgba(254, 127, 66, 0.4)";
...
// Panggil fungsi ini di goToStep
function goToStep(step) {
    ...
    // Update progress bar dengan animasi baru
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Efek hover untuk semua glass card
