    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <div class="flex gap-2">
                <a href="index.php" class="px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest <?= $filter_status == '' ? 'bg-red-600 text-white' : 'bg-white/5 text-gray-500 hover:bg-white/10' ?> transition-all flex items-center">Semua</a>
                ...
                <a href="index.php?f=Selesai" class="px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest <?= $filter_status == 'Selesai' ? 'bg-yellow-600 text-white' : 'bg-white/5 text-gray-500 hover:bg-white/10' ?> transition-all flex items-center">Selesai</a>
            </div>
        </div>
                                ...
                                <tr class="hover:bg-white/[0.02] transition-colors group">
                                    ...
                                    <td class="p-6 text-right">
                                        <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <!-- Ganti bagian tombol preview gambar di admin -->
<?php if (!empty($row['file_referensi'])): ?>
    <?php 
    $files = json_decode($row['file_referensi'], true);
    if (is_array($files) && count($files) > 0): ?>
        <div class="relative group">
            <button onclick="openFilePreview(<?= htmlspecialchars(json_encode($files), ENT_QUOTES) ?>)" 
                    class="p-3 bg-white/5 rounded-xl hover:bg-green-600 transition-all text-green-400 hover:text-white" 
                    title="Lihat Referensi (<?= count($files) ?> file)">
                <i data-lucide="folder-open" class="w-4 h-4"></i>
                <span class="absolute -top-1 -right-1 bg-blue-600 text-white text-[8px] px-1 rounded-full">
                    <?= count($files) ?>
                </span>
            </button>
        </div>
    <?php endif; ?>
<?php else: ?>
    <button class="p-3 bg-white/5 rounded-xl opacity-30 cursor-not-allowed" title="Tidak ada file">
...
<?php endif; ?>
    ...
    <div id="imageModal" class="fixed inset-0 z-[100] hidden p-4 backdrop-blur-sm" onclick="closePreview()">
        <div class="flex flex-col items-center justify-center w-full h-full" onclick="event.stopPropagation()">

            <button onclick="closePreview()" class="mb-4 text-white hover:text-red-500 transition-all flex items-center gap-2 font-bold uppercase tracking-widest text-xs">
                Tutup <i data-lucide="x" class="w-6 h-6"></i>
            </button>

            <img id="modalImg" src="" class="rounded-2xl shadow-2xl border border-white/10">
