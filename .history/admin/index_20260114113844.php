...
include '../includes/koneksi.php'; // Path naik satu level ke folder includes

// Ambil data projek terbaru
...
<body class="p-6 md:p-12">
    ...
    <td class="p-6 text-right">
        <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button onclick="showDetail('<?= addslashes($row['kebutuhan_detail']) ?>')" class="p-3 bg-white/5 rounded-xl hover:bg-blue-600 transition-all">
                <i data-lucide="eye" class="w-4 h-4"></i>
            </button>

            <?php if (!empty($row['nama_file'])): ?>
                <a href="../uploads/<?= $row['nama_file'] ?>" target="_blank" class="p-3 bg-white/5 rounded-xl hover:bg-green-600 transition-all text-green-400 hover:text-white" title="Lihat Foto">
                    <i data-lucide="image" class="w-4 h-4"></i>
                </a>
            <?php else: ?>
                <button class="p-3 bg-white/5 rounded-xl opacity-30 cursor-not-allowed" title="Tidak ada foto">
                    <i data-lucide="image-off" class="w-4 h-4"></i>
                </button>
            <?php endif; ?>

            <a href="edit_projek.php?id=<?= $row['id'] ?>" class="p-3 bg-white/5 rounded-xl hover:bg-white/20 transition-all">
                        ...
                        <?php if (mysqli_num_rows($query) > 0): ?>
                            <?php while ($row = mysqli_fetch_assoc($query)): ?>
                                <tr class="hover:bg-white/[0.02] transition-colors group">
                                    <td class="p-6">
                                        <div class="font-bold text-lg tracking-tight"><?= $row['nama_klien'] ?></div>
                                        <div class="text-sm text-gray-500 flex items-center gap-2 mt-1">
                                            <i data-lucide="mail" class="w-3 h-3"></i> <?= $row['email_klien'] ?>
                                        </div>
                                    ...
                                    <td class="p-6">
                                        <div class="text-sm font-semibold"><?= $row['jasa_pilihan'] ?></div>
