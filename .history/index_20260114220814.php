...
<head>
    ...
    <style>
        :root { --bg: #050810; --text: white; --card: rgba(255, 255, 255, 0.02); --border: rgba(255, 255, 255, 0.08); }
        .light-mode { --bg: #f8fafc; --text: #0f172a; --card: rgba(255, 255, 255, 0.8); --border: rgba(0, 0, 0, 0.05); }

        body { background-color: var(--bg); color: var(--text); font-family: 'Plus Jakarta Sans', sans-serif; transition: 0.4s; overflow-x: hidden; }
        .glass-card { background: var(--card); border: 1px solid var(--border); backdrop-filter: blur(15px); }
        .hero-gradient { background: radial-gradient(circle at top right, #1e293b, var(--bg)); }
        ...
        input, select, textarea { width: 100%; background: rgba(120, 120, 120, 0.05); border: 1px solid var(--border); border-radius: 1rem; padding: 1rem; color: inherit; outline: none; transition: 0.3s; }
...
<body class="min-h-screen hero-gradient flex items-center justify-center p-6">
    ...
    <div id="landing-page" class="active text-center max-w-2xl">
        <h1 class="text-6xl md:text-7xl font-black tracking-tighter mb-6 italic" id="txt-title">Z-STUDIO<span class="text-blue-600">.</span></h1>
            ...
            <form id="briefForm" action="simpan_brief.php" method="POST" enctype="multipart/form-data">
                ...
                <div id="step-1" class="step-content active space-y-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <input type="number" name="telepon" id="in-telp" placeholder="Nomor WhatsApp" required>
                    </div>
    ...
    <script>
        ...
        function toggleLang() {
            ...
            document.getElementById('in-telp').placeholder = dict.telp;
