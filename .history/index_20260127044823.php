...
<body class="min-h-screen hero-gradient flex items-center justify-center p-6">
    ...
    <div class="fixed top-6 right-6 flex gap-4 z-50">
        <button onclick="toggleLang()" class="glass-card px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all" id="langBtn">
            EN
        </button>
    ...
    <div id="landing-page" class="active text-center max-w-2xl">
        <h1 class="text-6xl md:text-7xl font-black tracking-tighter mb-6 italic" id="txt-title">
            Client<span class="text-blue-600">Brief.</span>
        </h1>
        <p class="text-gray-400 text-lg mb-10 font-medium" id="txt-desc">
            Ayo buat ide digital Anda jadi nyata
        </p>
    ...
    <div id="form-container" class="w-full max-w-2xl">
        ...
        <div class="glass-card rounded-[40px] p-8 md:p-12 shadow-2xl">
            <form id="briefForm" action="simpan_brief.php" method="POST" enctype="multipart/form-data">
                ...
                <div id="step-1" class="step-content active space-y-8">
                    <h2 class="text-3xl font-black italic uppercase tracking-tighter" id="txt-step1-title">
                        01. Identitas
                    </h2>
                    ...
                    <button type="button" onclick="validateAndNext(1, 2)" class="w-full bg-blue-600 text-white font-black py-5 rounded-[24px] uppercase tracking-tighter btn-next">
                        Lanjutkan
                    </button>
                </div>

                <div id="step-2" class="step-content space-y-8">
                    <h2 class="text-3xl font-black italic uppercase tracking-tighter" id="txt-step2-title">
                        02. Layanan
                    </h2>
                    ...
                    <div class="flex gap-4 justify-center">
                        <button type="button" onclick="goToStep(1)" class="flex-1 max-w-[30%] border border-white/10 text-gray-400 font-bold py-5 rounded-[24px] uppercase text-xs btn-back">
                            Kembali
                        </button>
                        <button type="button" onclick="validateAndNext(2, 3)" class="flex-1 max-w-[60%] bg-blue-600 text-white font-black py-5 rounded-[24px] uppercase tracking-tighter btn-next">
                            Lanjutkan
                        </button>
                    </div>
                </div>
                ...
                <div id="step-3" class="step-content space-y-8">
                    <h2 class="text-3xl font-black italic uppercase tracking-tighter" id="txt-step3-title">
                        03. Projek
                    </h2>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="border-2 border-dashed border-white/10 rounded-[24px] p-6 text-center relative bg-white/[0.02]">
                            <input type="file" name="gambar_projek[]" id="fileInput" accept=".jpg,.jpeg,.png,.pdf,.doc,.docx,.xls,.xlsx,.zip,.rar" multiple class="absolute inset-0 opacity-0 cursor-pointer">
                            <i data-lucide="upload-cloud" class="w-8 h-8 text-blue-500 mx-auto mb-2"></i>
                            <p class="text-[10px] font-bold text-gray-500 uppercase" id="txt-upload">
                                Upload Referensi (Multiple Files Allowed)
                            </p>
                            <p class="text-xs text-gray-400 mt-2" id="fileInfo">Max 10MB per file</p>
                            <div id="fileList" class="mt-4 space-y-2 text-left"></div>
                        </div>
                    </div>

                    <div class="flex gap-4 justify-center">
                        <button type="button" onclick="goToStep(2)" class="flex-1 max-w-[30%] border border-white/10 text-gray-400 font-bold py-5 rounded-[24px] uppercase text-xs btn-back">
                            Kembali
                        </button>
                        <button type="submit" name="submit" class="flex-1 max-w-[60%] bg-white text-black font-black py-5 rounded-[24px] hover:bg-blue-600 hover:text-white transition-all uppercase tracking-tighter" id="txt-submit">
                            Kirim Brief
                        </button>
                    </div>
                </div>
