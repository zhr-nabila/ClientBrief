...
// Fungsi untuk membatalkan pengisian (Kembali ke Hero)
function cancelBrief() {
    // Sembunyikan Form
    document.getElementById('form-section').classList.add('hidden');
    // Munculkan kembali Hero dan Service
    document.getElementById('hero').classList.remove('hidden');
    document.getElementById('services').classList.remove('hidden');
    
    // Reset Form (Opsional, agar data terhapus saat cancel)
    document.querySelector('form').reset();
    nextStep(1); // Balikkan progress ke step 1
    window.scrollTo({ top: 0, behavior: 'smooth' });
}

// Tambahkan event listener pada link "Beranda" di navbar agar bisa cancel otomatis
document.querySelector('[data-lang="nav-home"]').addEventListener('click', (e) => {
    if (!document.getElementById('form-section').classList.contains('hidden')) {
        cancelBrief();
    }
});