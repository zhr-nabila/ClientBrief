...
<body class="bg-gray-950 text-white overflow-x-hidden">
   ...
   <section id="form-section" class="hidden py-32 px-6">
    <div class="max-w-2xl mx-auto relative">
        ...
        <button onclick="cancelBrief()" class="absolute -top-12 right-0 flex items-center gap-2 text-gray-400 hover:text-white transition-all group">
        ...
        <div class="bg-gray-900/50 backdrop-blur-md p-10 rounded-[40px] border border-white/10 shadow-2xl">
            ...
            <form action="simpan_brief.php" method="POST" enctype="multipart/form-data">
                <div class="step active">
                    ...
                    <button type="button" onclick="nextStep(2)" class="w-full bg-blue-600 py-4 rounded-2xl font-bold hover:bg-blue-700 transition" data-lang="btn-next">Lanjut</button>
                </div>
                ...
                <div class="step">
                    ...
                    <div class="grid gap-3">
                        ...
                        <label class="flex items-center p-4 border border-gray-700 rounded-2xl cursor-pointer hover:bg-blue-600/20 transition group">
                            <input type="radio" name="jasa" value="<?= $s ?>" class="hidden peer" required>
                            <span class="w-4 h-4 border border-gray-500 rounded-full mr-4 peer-checked:bg-blue-500 peer-checked:border-blue-500 transition-all"></span>
                            <span class="font-medium group-hover:text-blue-400 transition-all"><?= $s ?></span>
                        </label>
                        <?php endforeach; ?>
                    </div>
                    <div class="flex gap-4 mt-8">
                        <button type="button" onclick="nextStep(1)" class="w-1/3 text-gray-400 hover:text-white transition" data-lang="btn-back">Kembali</button>
                        <button type="button" onclick="nextStep(3)" class="w-2/3 bg-blue-600 py-4 rounded-2xl font-bold hover:bg-blue-700 transition" data-lang="btn-next">Lanjut</button>
                    </div>
                </div>

                <div class="step">
                    <h2 class="text-2xl font-bold mb-6" data-lang="form-step-3-title">Detail</h2>
                    <textarea name="pesan" rows="5" class="w-full p-4 bg-black/30 border border-gray-700 rounded-2xl mb-4 outline-none focus:border-blue-500 transition" placeholder="Jelaskan kebutuhan Anda..."></textarea>
                    <div class="mb-6">
                        <label class="block text-xs text-gray-500 mb-2 uppercase tracking-widest font-bold" data-lang="form-label-file">Upload Lampiran</label>
                        <input type="file" name="file_brief" class="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-gray-800 file:text-gray-300 hover:file:bg-gray-700 transition-all">
                    </div>
                    <div class="flex gap-4">
                        <button type="button" onclick="nextStep(2)" class="w-1/3 text-gray-400 hover:text-white transition" data-lang="btn-back">Kembali</button>
                        <button type="submit" name="submit" class="w-2/3 bg-green-600 py-4 rounded-2xl font-bold hover:bg-green-700 transition shadow-lg shadow-green-900/20" data-lang="btn-submit">Kirim</button>
                    </div>
                </div>
            </form>
        </div>
